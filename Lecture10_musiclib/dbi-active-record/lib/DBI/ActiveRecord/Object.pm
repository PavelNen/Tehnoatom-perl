...
Каждый такой метод, получает экземпляр класса-адаптера для доступа к БД
через поле C<db_class> мета-класса объекта, и вызывает у него соответсвующий метод для работы с БД.
...
Метод класса для выборки данных из БД по полю С<$field>,
которое может принимать значения/значение C<$keys>.
Параметр C<$limit> определяет лимит выборки,
при запросах в БД по неуникальному ключу.
