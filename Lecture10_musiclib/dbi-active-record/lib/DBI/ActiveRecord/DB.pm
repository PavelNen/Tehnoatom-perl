...
Данный класс используется непосредственно для работы с БД.
Его абстрактность обусловленна тем, что различные СУБД требуют различных подходов,
по работе с ними. Вся специфическая работа, непосредственно с СУБД
должна определяться в классах наследниках от C<DBI::ActiveRecord::DB>
(например, см. C<DBI::ActiveRecord::DB::SQLite>).

Данный класс реализует паттерн синглтон - объект создается один раз,
при первом обращении к классу, и используется при всех последующих обращениях.
...
Значение атрибута, может быть автоматически сгененрированно, путём определения
...
Метод для выбоки из БД, вызывающийся из наследников класса C<DBI::ActiveRecord::Object>.
C<$klass> - класс ActiveRecord объекта, который будет использован для конструирования.

Значения ключевого поля перед передачей в SQL запрос, должны сериализовываться
фунцией C<serialize>, если она оперделена для данного атрибута.

Для ключевого поля должен быть определен атрибут C<index>,
в противном случае будет вызвано исключение!

Значения полей полученных из БД, должны быть десериализованны
фунциями C<deserialize>, если таковые определены для этих атрибутов.
...
Значения первичного ключа и остальных полей перед передачей в SQL запрос,
должны сериализовываться фунциями C<serialize>, если они оперделены для данных атрибутов.
