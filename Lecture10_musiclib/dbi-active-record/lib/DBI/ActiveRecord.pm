...
При подключении модуля C<DBI::ActiveRecord>, подключающий пакет,
автоматически становится наследником класса C<DBI::ActiveRecord::Object>,
а так же подключает к вашему классу трейт C<DBI::ActiveRecord::Trait::Class>.
...
Данный метод - Mouse "надстройка" над методом C<import>.
Он вызывается при подключении C<Mouse> модуля через C<use>.

Чаще всего это используется для подмены базового класса ваших C<Mouse>-объектов,
а так же добавления ролей к мета-классу.
...
Функция принимает, в качестве первого параметра, имя атрибута,
которое должно совпадать с именем колонки в таблице.

Далее передается хеш-параметров, описывающих атрибут.
Функция создает C<Mouse>-атрибут с указанным именем и параметрами в вашем объекте.

В качестве параметров можно передать любые параметры, которые можно передать
в C<Mouse>-функцию C<has>, за исключением параметра C<is> - его значение
игнорируется и атрибут создается всегда доступный для чтения и записи.
